...
fs.writeFileSync(path.join(__dirname, "../src/bgfx.cpp"), bindings_cpp);

const bindings_hpp = lua.lua_tojsstring(L, -2);
fs.writeFileSync(path.join(__dirname, "../src/bgfx.hpp"), bindings_hpp);

const bindings_js = lua.lua_tojsstring(L, -1);
fs.writeFileSync(path.join(__dirname, "../bgfx.js"), bindings_js);
