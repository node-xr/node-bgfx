...
class VertexArray {
  constructor(spec, length, data) {
    ...
    if (data && length) {
      ...
      this.length = length;
      this._data = new ArrayBuffer(stride * length);
    } else {
  ...
  clone() {
    return new VertexArray(this._spec, this.length, this._data.slice(0));
  }
}
...
class VertexBuffer {
  constructor(decl, length, data, options) {
    ...
    this.options = {
      ...
      flags: (options && options.flags) || constants.BUFFER_NONE,
  ...
  upload() {
    if (!this.handle) {
      ...
      bgfx.update_dynamic_vertex_buffer(this.handle, 0, this.array._data);
    } else {
  ...
  clone() {
    ...
    return new VertexBuffer(decl, length, array._data.slice(0), options);
  }
   ...
   *
   * @param data the object containing one array per key.
   */
...
class VertexDeclaration {
  ...
  wrap(data, options) {
    // Validate that the data array makes sense.
    let length = undefined;
    for (const field of this._spec) {
      // Check that all required fields are present.
      const { attr } = field;
      if (!(attr in data))
        throw new Error(`Missing data attribute '${field.attr}'`);
      ...
      if (!arr.length) throw new Error(`'${attr}' cannot have empty array.`);
        ...
        throw new Error(
          `'${field.attr}' array was length ${arr.length}, expected ${length}.`,
        );
    }

    // Create a vertex buffer for this data.
    const buffer = this.allocate(length, options);
    for (const [attr, arr] of Object.entries(data)) {
      for (let i = 0; i < buffer.length; ++i) {
        buffer.array.get(i)[attr].set(arr[i]);
      }
